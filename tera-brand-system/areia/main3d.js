...
import { loadWordmark, fitWordmark, rasterizeWordmark } from './wordmark.js?v=17';
import { SandVolume } from './sand3d.js?v=17';
import { Scene3D } from './scene3d.js?v=17';
import { PALETTES, buildLut, lutIndex } from './palette.js?v=17';
import { fromSearch, toSearch } from './params.js?v=17';
import { Panel } from './panel.js?v=17';
...
function rainPerStep() {
  ...
  if (state.fill) {
    const left = state.fill.target - volume.settled;
...
function startFill() {
  resetScene();
  state.fill = {
    target: Math.min(MAX_SETTLED, Math.round(volume.capacity * (values.nivel / 100))),
...
function updateHud() {
  ...
  if (state.fase === 'enchendo' && state.fill) {
    parts.push(`${Math.min(99, Math.round((volume.settled / state.fill.target) * 100))}%`);
  }
